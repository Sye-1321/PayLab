...
ALTER TABLE run_events ADD CONSTRAINT run_events_event_type_check CHECK (event_type IN (
    'RUN_STARTED', 'MERCHANT_REQUEST_OBSERVED', 'PRE_COMMIT_TIMEOUT_INJECTED',
    'PAYMENT_COMMITTED', 'RESPONSE_DELAY_INJECTED', 'MERCHANT_STATUS_QUERY_OBSERVED',
    'WEBHOOK_SCHEDULED', 'WEBHOOK_DELIVERED', 'WEBHOOK_RESPONSE_OBSERVED'
));
ALTER TABLE run_events ADD CONSTRAINT run_events_fields_check CHECK (
    ...
    OR (event_type = 'PRE_COMMIT_TIMEOUT_INJECTED' AND idempotency_key IS NOT NULL
        AND request_fingerprint IS NOT NULL AND webhook_event_id IS NULL
        AND http_status IS NULL AND outcome IS NULL AND payment_id IS NULL
        AND response_delay_millis BETWEEN 1 AND 30000)
...
CREATE UNIQUE INDEX run_events_one_pre_commit_timeout_per_run
    ON run_events(run_id) WHERE event_type = 'PRE_COMMIT_TIMEOUT_INJECTED';
