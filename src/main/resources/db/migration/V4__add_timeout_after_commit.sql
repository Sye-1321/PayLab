ALTER TABLE test_runs ADD COLUMN response_delay_millis INTEGER;
ALTER TABLE test_runs ADD CONSTRAINT test_runs_scenario_configuration_check CHECK (
    ...
    OR (scenario_id = 'TIMEOUT_AFTER_COMMIT' AND webhook_url IS NULL
        AND response_delay_millis BETWEEN 1 AND 30000)
);

ALTER TABLE run_events DROP CONSTRAINT run_events_fields_check;
ALTER TABLE run_events
    ADD COLUMN payment_id VARCHAR(64) REFERENCES provider_payments(payment_id),
    ADD COLUMN response_delay_millis INTEGER;
